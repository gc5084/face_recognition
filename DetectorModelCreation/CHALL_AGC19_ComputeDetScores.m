function [FD_score, scoresSTR] = CHALL_AGC19_ComputeDetScores(...
    DetectionSTR, AGC19_Challenge1_STR, show_figures)
%
% Compute face detection score
%
% [FD_score, scoresSTR] = CHALL_AGC19_ComputeDetScores(...
%    DetectionSTR, AGC19_Challenge1_STR, show_figures)
%
% INPUTS
...
%   AGC19_Challenge1_TRAINING or AGC19_Challenge1_TEST).
%
...
scoresSTR = struct();
for j = 1 : length( AGC19_Challenge1_STR )
    if show_figures
        A = imread( AGC19_Challenge1_STR(j).imageName );    
        
        clf;
        imshow( A, 'InitialMagnification', 'fit');
        hold on;
        
        for k1 = 1 : size( AGC19_Challenge1_STR(j).faceBox, 1 )
            fb = AGC19_Challenge1_STR(j).faceBox( k1, : );
    ...
    n_actualFaces = size( AGC19_Challenge1_STR(j).faceBox, 1 );
    ...
    if not( n_actualFaces )
        ...
        if not( n_detectedFaces )
            ...
            for k1 = 1 : n_actualFaces
                f = AGC19_Challenge1_STR(j).faceBox(k1, :);
